...
var rootLog, _ = logging.RootLogger("firesui", "github.com/ticketland-io/firehose-sui/cmd/firesui/cli")

var RootCmd = &cobra.Command{Use: "firesui", Short: "Sui on StreamingFast"}
var allFlags = make(map[string]bool) // used as global because of async access to cobra init functions

func Main() {
	cobra.OnInitialize(func() {
		allFlags = flags.AutoBind(RootCmd, "firesui")
	})
	...
	derr.Check("sui-blockchain", RootCmd.Execute())
}

var startCmdExample = `firesui start reader-node`
